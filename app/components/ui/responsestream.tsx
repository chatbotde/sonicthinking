...
export type ResponseStreamProps = {
  ...
  renderText?: (text: string) => React.ReactNode // Optional custom text renderer
...
function ResponseStream({
  ...
  const {
    ...
    segmentDelay,
    characterChunkSize,
    onError, // Pass onError to the hook
  })
  ...
  const renderContent = () => {
    switch (mode) {
      case "typewriter":
        // If textStream is a string (meaning context is handling updates), render it directly.
        // Otherwise, use the hook's displayedText for AsyncIterable streams.
        if (typeof textStream === "string") {
          return renderText ? renderText(textStream) : <>{textStream}</>;
        } else {
          // Fallback to hook's state for non-string streams (if any)
          return renderText ? renderText(displayedText) : <>{displayedText}</>;
        }

      case "fade":
        // Fade logic remains unchanged and uses the hook's state
        return (
          <>
